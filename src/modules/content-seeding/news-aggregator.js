...
function getRoomSpecificNews (): Promise<Array<RoomSpecificNews>> {
	return performReadQuery({
		$: `SELECT rooms.id as roomid, article.rawjson as article, article.url as url FROM rooms 
			JOIN LATERAL (
...
export const newsAggregator = async () => {
	const truncateResponse = await performWriteQuery([ {
		$: 'TRUNCATE TABLE articles'
	} ]);
	console.log(truncateResponse);
